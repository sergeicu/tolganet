...
def filter_b0(dir_files, HCP):
    ...
    sample =dir_files + "/{}_3T_DWI_dir95_LR.bval".format(HCP)
    #print(sample)
    
    #bval_b0 = read_bvals(glob.glob(sample))
    bval_b0 = read_bvals(os.path.normpath(sample))
    bval_b0 = np.where(bval_b0 < 100)
    bval_b0 = np.asarray(bval_b0).astype("int")[0]
    return bval_b0

dir_main = "/home/ch253549/Downloads/FD-Net/FD-Net/data"
#print(dir_main)
...
for HCP in val:
    ...
    b_iter = filter_b0(dir_files, HCP)
        ...
        # TOPUP to slices
    topup_files = glob.glob(dir_files + "/*topup_results.nii*")
    ...
    for i in range(len(topup_files)):
        ...
        save_name = dir_main + "/val/val/slices_topup/" + file_name
    ...
    # TOPUP field to slices
    topup_files = glob.glob(dir_files + "/*topup_fout.nii*")
    ...
    for i in range(len(topup_files)):
        ...
        # file_name = topup_file.split("/")[-1]
        # # print(file_name)
        # file_name = file_name[:file_name.find("_LR.nii")]

        #file_name = LR_file.split("/")[-1][:file_name.find("_LR.nii")]

        save_name = dir_main + "/val/val/slices_field_topup/" + file_name
    ...
    # LR & RL to stacked slices
    LR_files = sorted(glob.glob(dir_files + "/*LR.nii*"))
    RL_files = sorted(glob.glob(dir_files + "/*RL.nii*"))
    ...
    for i in range(len(LR_files)):
        ...
        save_name = dir_main + "/val/val/slices_like_topup/" + file_name
        # print(LR_file)
        # print(RL_file)
        LR_data = nib.load(LR_file).get_fdata()
        RL_data = nib.load(RL_file).get_fdata()
...
for HCP in test:
    ...
    for i in range(len(topup_files)):
        ...
        save_name = dir_main + "/test/test/slices_topup/" + file_name
    ...
    for i in range(len(topup_files)):
        ...
        # save_name = dir_main + "/test/test/slices_field_topup/" + file_name
        # file_name = LR_file.split("/")[-1]
        # # print(file_name)
        # file_name = file_name[:file_name.find("_LR.nii")]
    ...
    for i in range(len(LR_files)):
        ...
        save_name = dir_main + "/test/test/slices_like_topup/" + file_name
        # print(LR_file)
        # print(RL_file)
        LR_data = nib.load(LR_file).get_fdata()
        RL_data = nib.load(RL_file).get_fdata()
