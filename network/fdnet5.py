...
if __name__ == '__main__':
    ...
    dg_train, dg_val, dg_test,list_test = load_data(data_path, args.batch_size, args.debug,args.dataset)
    ...
    if args.mode == 'train':
        ...
        #callback4 = BatchValidationCallback(dg_val, batch_freq=10)
        ...
        for j in tqdm(range(len(dg_test))):
            ...
            # save the files? 
            imo=nb.load(list_test[j])
            savedir = os.path.dirname(weights) + "/" 
            os.makedirs(savedir +"predicted/", exist_ok=True)
            nb.save(nb.Nifti1Image(XLR.numpy(),affine=imo.affine,header=imo.header), savedir+"predicted/"+file_name.replace(".nii.gz", "_XLR.nii.gz"))
            nb.save(nb.Nifti1Image(XRL.numpy(),affine=imo.affine,header=imo.header), savedir+"predicted/"+file_name.replace(".nii.gz", "_XRL.nii.gz"))
            nb.save(nb.Nifti1Image(YLR,affine=imo.affine,header=imo.header), savedir+"predicted/"+file_name.replace(".nii.gz", "_YLR.nii.gz"))
            nb.save(nb.Nifti1Image(YRL,affine=imo.affine,header=imo.header), savedir+"predicted/"+file_name.replace(".nii.gz", "_YRL.nii.gz"))
            nb.save(nb.Nifti1Image(topup_image.numpy(),affine=imo.affine,header=imo.header), savedir+"predicted/"+file_name.replace(".nii.gz", "_topup_image.nii.gz"))
            nb.save(nb.Nifti1Image(network_image,affine=imo.affine,header=imo.header), savedir+"predicted/"+file_name.replace(".nii.gz", "_network_image.nii.gz"))
            nb.save(nb.Nifti1Image(topup_field.numpy(),affine=imo.affine,header=imo.header), savedir+"predicted/"+file_name.replace(".nii.gz", "_topup_field.nii.gz"))
            nb.save(nb.Nifti1Image(network_field,affine=imo.affine,header=imo.header), savedir+"predicted/"+file_name.replace(".nii.gz", "_network_field.nii.gz"))
            with open(savedir+"predicted/"+file_name.replace(".nii.gz","_rigid_transform.txt"), "w") as f:
        ...
        ## This is supposed to print test metrics... 
